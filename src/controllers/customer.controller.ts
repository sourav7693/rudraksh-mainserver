...
export const checkMobileExists = async (
  ...
  try {
    ...
    // Ensure format starts with 91
    if (!mobile.startsWith("91")) {
      mobile = "91" + mobile;
    }

    // Logged-in customer id (from protect middleware)
    const customerId = req.user?.id;

    // Find if mobile exists for some OTHER customer
    const existingCustomer = await Customer.findOne({
      mobile,
      _id: { $ne: customerId }, // exclude current user
    });
